...
rm cedar_files.txt
globus ls $source_ep:projects/rrg-eros-ab/ekoch/VLAXL/calibrated/ >> cedar_files.txt
...
task_id="$(globus transfer $source_ep:projects/rrg-eros-ab/ekoch/VLAXL/calibrated/ $dest_ep:space/vlaxl/calibrated/ --jmespath 'task_id' --format=UNIX --batch < batch_files.txt)"

echo "Waiting on 'globus transfer' task '$task_id'"
globus task wait "$task_id" --polling-interval 300
if [ $? -eq 0 ]; then
